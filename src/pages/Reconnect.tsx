  ...
  <section className={shellCardClass}>
    ...
    <div className="mt-4 space-y-2">
      {reconnectTools.map((tool) => {
        const isSelected = tool.slug === selectedSlug;
        return (
          <button
            key={tool.slug}
            type="button"
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="font-body text-sm text-foreground">{tool.title}</div>
                <div className="mt-1 text-xs leading-5 text-muted-foreground">{tool.overviewLine}</div>
              </div>
...
const FreeReconnectContent = ({ tool }: { tool: ReconnectTool }) => {
  ...
  return (
    <main className="space-y-5">
      ...
      <section className="rounded-[24px] border border-border/30 bg-background/45 p-5">
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">8. Related Paths & Authors</p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {data.related.map((entry) => (
            <article key={`${entry.kind}-${entry.name}`} className="rounded-2xl border border-border/25 bg-card/35 p-4">
              <p className="text-[11px] uppercase tracking-[0.14em] text-primary/80">{entry.kind}</p>
              <h4 className="mt-2 font-body text-sm text-foreground">{entry.name}</h4>
              <p className="mt-2 text-xs leading-6 text-muted-foreground">{entry.note}</p>
            </article>
          ))}
        </div>
      </section>
    ...
    <div className="space-y-6">
      ...
      <section className="grid items-start gap-8 lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="space-y-4 lg:sticky lg:top-24">
          ...
          <MoreReconnectCard selectedSlug={selectedSlug} onSelect={setSelectedSlug} />
          <PremiumMiniCard />
        </aside>
